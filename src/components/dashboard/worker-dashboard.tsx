...
const WorkerDashboard = ({
  ...
  const workplaceNames =
    workplaces?.map((workplace) => workplace?.name).filter(Boolean) ?? [];
    ...
    <div className="min-h-screen bg-gradient-to-br from-zinc-950 via-zinc-900 to-zinc-950 text-foreground">
      ...
      <main className="mx-auto flex max-w-5xl flex-col gap-8 px-6 py-10">
        <section className="grid gap-6 md:grid-cols-[2fr_3fr]">
          ...
          <ClockControls
            hasActiveShift={Boolean(activeEntry)}
        ...
        <Card className="border border-white/5 bg-black/40">
          <CardHeader>
            ...
            <CardDescription>
              Locations you are allowed to clock into.
            </CardDescription>
          </CardHeader>
          <CardContent className="grid gap-4">
            {workplaces?.length ? (
              ...
              <p className="text-sm text-muted-foreground">
                You have no workplaces assigned.
              </p>
            )}
          </CardContent>
        </Card>
                ...
                <TableBody>
                  {recentEntries.map((entry) => {
                    ...
                    const duration = clockOut
                      ? formatDistanceStrict(clockOut, clockIn, {
